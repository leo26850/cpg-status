...
${renderKpiBar(r, fmt)}
${renderMonthlyTable(r, fmt)}
${renderBySource(r, fmt)}
${renderMql(r, fmt)}
${renderSql(r, fmt)}
${renderCohort(r, fmt)}
${renderCpaPayback(r, fmt)}
${renderLeadLog(r)}
${renderDefinitions()}
...
function renderKpiBar(r: ReportData, fmt: (n: number | null, opts?: { currency?: boolean; pct?: boolean }) => string): string {
  ...
  return `<section><div class="container">
    ...
    <div class="metric-grid">
      ...
      <div class="stat"><div class="stat-num">${fmt(k.cpl, { currency: true })}</div><div class="stat-label">CPL</div></div>
      <div class="stat"><div class="stat-num">${fmt(k.cpa, { currency: true })}</div><div class="stat-label">CPA</div></div>
    </div>
  </div></section>`;
}
...
function renderMql(r: ReportData, fmt: (n: number | null, opts?: { currency?: boolean; pct?: boolean }) => string): string {
  const rows = r.monthly.map((m) => `
    <tr>
      ...
      <td>${fmt(m.cpl_to_mql, { currency: true })}</td>
    </tr>`).join('');
  return `<section><div class="container">
    ...
    <table style="margin-top: 24px;">
      <thead><tr><th>Month</th><th>MQL (gads)</th><th>MQL (bison)</th><th>MQL Rate</th><th>CPL → MQL</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </div></section>`;
}
...
function renderDefinitions(): string {
  return `<section><div class="container">
    ...
    <div class="grid-2">
      ...
      <div class="card"><h4>CPL</h4><p>Fully-loaded monthly spend ÷ deduped, junk-excluded leads. First-touch attribution.</p></div>
      <div class="card"><h4>CPA</h4><p>Cohort-based: N−3 month spend ÷ Closed Won deals from the cohort by month N+90.</p></div>
    </div>
